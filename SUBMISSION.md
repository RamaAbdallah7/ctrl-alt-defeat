...
**Tarteeb — the project-management copilot that lives in your Slack channel**
...
Tarteeb puts the analysis where the conversation already is. You `/pm` a
...
| 1:10–1:30 | `@Tarteeb` with budget figures; consolidated brief with schedule + cost + vendor | "And when I give it budget figures too, one brief consolidates all three specialists — and catches that we're over budget *and* behind schedule on the critical path." |
...
| 1:45–2:00 | terminal: `python demo/cli_demo.py audit` scrolling the JSONL | "Every step — what was asked, what each engine returned, the brief, the human's decision — is in an append-only audit trail. Advice you can trace. That's Tarteeb." |
